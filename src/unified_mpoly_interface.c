...
/* ============================================================================
   MULTIPLICATION WITH ZECH OPTIMIZATION
   ============================================================================ */
...
int unified_mpoly_mul(unified_mpoly_t poly1, const unified_mpoly_t poly2,
    ...
    switch (poly1->field_id) {
        ...
        case FIELD_ID_FQ_ZECH:
            if (use_zech_mul) {
                /* Use optimized Zech multiplication */
...
/* ============================================================================
   ENHANCED DIVISION OPERATIONS WITH ZECH SUPPORT
   ============================================================================ */

/* Global flags for division optimizations */
static int use_gf28_div_opt = 0;
static int use_gf2128_div_opt = 0;
static int use_zech_div = 1;  /* Enable Zech division by default */
...
int unified_mpoly_divides(unified_mpoly_t Q, const unified_mpoly_t A,
    ...
    switch (Q->field_id) {
        ...
        case FIELD_ID_FQ_ZECH:
            if (use_zech_div) {
                /* Use Zech logarithm division - should be very fast */
        ...
        case FIELD_ID_GF28:
            if (1 || use_gf28_div_opt) {
                ...
                /* TIMING START */
                double start_time = get_wall_time_8();
                //printf("begin division\n");
                
                /* Try optimized division */
                int success = gf28_mpoly_divides(Q_native, A_native, B_native, native_ctx); //gf28_mpoly_divides gf28_mpoly_divides_sparse
                
                //printf("end division\n");
                double end_time = get_wall_time_8();
                //printf("GF(2^8) division time: %.6f seconds\n", end_time - start_time);
                /* TIMING END */
                if (success) {
                    /* Convert result back to fq_nmod format */
